...
import dominio.dto.request.NoticiaRequestDTO;
...
@Tag(name = "Noticia", description = "Gerenciamento de Noticias") // Agrupa endpoints no Swagger sob a tag "Noticia"
...
public class NoticiaController {
    ...
    public ResponseEntity<NoticiaResponse> criarArtigo(
            @Valid @RequestBody NoticiaRequestDTO request // Corpo da requisição será validado e mapeado para o DTO
    ) {
        // Chama o service para criar o artigo a partir do DTO recebido
        NoticiaResponse artigo = artigoService.criarArtigo(request);
    ...
    public ResponseEntity<Page<NoticiaResponse>> listarArtigos(
            ...
            @RequestParam(required = false) StatusNoticia status, // Filtro opcional por status
    ...
    public ResponseEntity<NoticiaResponse> buscarPorId(@PathVariable Long id) {
        ...
        NoticiaResponse artigo = artigoService.buscarPorId(id);
    ...
    public ResponseEntity<NoticiaResponse> atualizarArtigo(
            @PathVariable Long id, // ID do artigo na URL
            @Valid @RequestBody NoticiaRequestDTO request // Dados novos do artigo vindos no corpo da requisição
    ) {

        // Chama o serviço para atualizar o artigo com o ID informado
        NoticiaResponse artigoAtualizado = artigoService.atualizarArtigo(id, request);
    ...
    public ResponseEntity<NoticiaResponse> publicarArtigo(@PathVariable Long id) {
        // Chama o serviço para publicar o artigo (normalmente altera status para PUBLICADO e seta dataPublicacao)
        NoticiaResponse artigo = artigoService.publicarArtigo(id);
    ...
    public ResponseEntity<List<NoticiaResponse>> estatisticas() {
        // Lista todos os artigos incluindo campos estatísticos adicionais (views, avaliações, etc.)
        List<NoticiaResponse> lista = artigoService.listarTodosComEstatisticas();
    ...
    public ResponseEntity<Page<NoticiaResponse>> listarPublicados(
            @RequestParam(defaultValue = "0") int page, // Página atual (default 0)
        ...
        Page<NoticiaResponse> artigos = artigoService.listarPublicados(pageable);
